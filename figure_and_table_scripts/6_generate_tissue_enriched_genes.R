...
reference_atlas <- read.delim("datasets/reference_atlas.tsv",header = T,stringsAsFactors = F)
...
for(stage in 1:length(timepoints)){
  ...
  for(tissue in names(tissues)){
    ...
    foldchanges <- log2(rowMeans(subreference_atlas[,tissuereps]+.1)/rowMeans(subreference_atlas[,nottissuereps]+.1))
