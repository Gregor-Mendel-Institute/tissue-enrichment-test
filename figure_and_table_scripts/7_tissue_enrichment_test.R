...
#          test_timepoint- developmental timepoint to compare to. Options in 'timepoints.txt'
    ...
    plot(ecdf(percentile_seqdata),lwd=4,xlim=c(0,100),ylim=c(0,1),las=1,xlab='Percentile',ylab='Cumulative Frequency',main='')
    for(i in 1:length(enriched[[test_timepoint]])){
      ...
      lines(ecdf(values_E),lwd=2,col=tissue_colors[tissue],cex=.5)
    }
    lines(ecdf(percentile_seqdata),lwd=4)
...
perform_contamination_test <- function(data_table,columns,timepoints,gaps_row,gaps_col,output_name,make_heatmaps){
  ...
  if(make_heatmaps){
    ...
    pheatmap(-log10(pvalue_matrix[1:7,]),cluster_rows = F,cluster_cols = F,scale = 'none', breaks = breaksList,color = colorRampPalette(rev(brewer.pal(n = 10, name = "RdYlBu")))(length(breaksList)),
             border_color = 'black',gaps_col = gaps_col,
             gaps_row = gaps_row,main="Tissue-Specific Transcript Enrichment",
             labels_row = tissues[rownames(pvalue_matrix)],
